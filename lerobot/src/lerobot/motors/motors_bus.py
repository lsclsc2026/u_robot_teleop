...
class SerialMotorsBus(MotorsBusBase):
    ...
    def torque_disabled(self, motors: str | list[str] | None = None, num_retry: int = 0):
        ...
        Args:
            motors: Motors to disable, or all configured motors when omitted.
            num_retry: Additional communication attempts for each torque transition.
    ...
    def set_half_turn_homings(
        ...
        # USB serial links forwarded through WSL/usbip can occasionally miss one
        # status packet. Calibration is interactive and should tolerate a short
        # transient instead of aborting after a single failed read.
        actual_positions = self.sync_read(
            "Present_Position", motor_names, normalize=False, num_retry=5
        )
